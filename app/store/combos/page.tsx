...
export default function CombosPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const { data: combosData } = await supabase
        .from('combos')
        .select('*, combo_items(*, products(name, sale_price))')
        .eq('store_id', store.id)
  ...
  const valorNormal = selectedItems.reduce(
    ...
    <div>
      ...
      {combos.length === 0 ? (
        ...
        <div className="space-y-3">
          {combos.map((combo) => {
            const valorNorm = combo.items.reduce(
              (s, i) => s + i.unit_price * i.quantity,
              0
            )
            ...
            return (
              <div
                key={combo.id}
                className={`bg-white rounded-xl shadow-sm border p-4 lg:p-5 ${
                  !combo.is_active ? 'opacity-60' : 'border-gray-100'
                }`}
              >
                  ...
                  {valorNormal > 0 && form.price && (
                    <p className="text-xs mt-1">
                      ...
                      <span className="line-through text-gray-400">
                        S/ {valorNormal.toFixed(2)}
                      </span>
                      {parseFloat(form.price) < valorNormal && (
                        <span className="text-green-600 font-medium ml-1">
                          · Ahorro: S/ {(valorNormal - parseFloat(form.price)).toFixed(2)}
                        </span>
                      )}
                    </p>
                  )}
                ...
                {selectedItems.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {selectedItems.map((item) => (
                      <div
                        key={item.product_id}
                        className="flex items-center gap-3 bg-blue-50 rounded-xl px-3 py-2"
                      >
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            ...
                            className="w-6 h-6 bg-white rounded-lg text-blue-600 font-bold text-sm flex items-center justify-center"
                          >
                            −
                          </button>
                  ...
                  <>
                    ...
                    <div className="space-y-1 max-h-48 overflow-y-auto border border-gray-100 rounded-xl p-2">
                      {products
                        ...
                        .map((product) => {
                          const inCombo = selectedItems.find(
