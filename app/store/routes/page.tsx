...
export default function RoutesPage() {
  ...
  const loadOrders = async () => {
    try {
      ...
      if (store.origin_lat && store.origin_lng) {
        setStoreOrigin({ lat: store.origin_lat, lng: store.origin_lng })
      }
  ...
  const generateRoute = async () => {
    ...
    if (ordersWithLocation.length === 0) {
      alert('Ningún pedido seleccionado tiene coordenadas GPS')
      setOptimizing(false)
      return
    }
    ...
    if (storeOrigin) {
      const originIcon = L.divIcon({
        className: '',
        html: `<div style="background:#16a34a;color:white;border-radius:50%;width:32px;height:32px;display:flex;align-items:center;justify-content:center;font-size:16px;border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.4)">🏪</div>`,
        iconSize: [32, 32],
        iconAnchor: [16, 16],
      })

      markersRef.current.push(
        L.marker([storeOrigin.lat, storeOrigin.lng], { icon: originIcon })
          .addTo(map)
          .bindPopup('<b>Punto de salida</b>')
      )
    }
    ...
    ordersWithLocation.forEach((order, i) => {
      const icon = L.divIcon({
        className: '',
        html: `<div style="background:#2563eb;color:white;border-radius:50%;width:30px;height:30px;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:bold;border:2px solid white;box-shadow:0 2px 6px rgba(0,0,0,0.4)">${i + 1}</div>`,
        iconSize: [30, 30],
        iconAnchor: [15, 15],
      })
      ...
      markersRef.current.push(
        L.marker([order.lat!, order.lng!], { icon })
          .addTo(map)
          .bindPopup(`<b>${order.order_code}</b><br>${order.customer_name}`)
      )
    ...
    try {
      ...
      if (coordPoints.length >= 2) {
        ...
        if (data.code === 'Ok' && data.trips?.[0]) {
          ...
          const geojsonCoords = trip.geometry.coordinates.map(
            (c: number[]) => [c[1], c[0]] as [number, number]
          )

          routeLayerRef.current = L.polyline(geojsonCoords, {
            color: '#2563eb',
            weight: 4,
            opacity: 0.8,
            dashArray: '8, 4',
          ...
          const sortedByOsrm = ordersWithLocation
            .map((o, i) => ({
              order: o,
              osrmIndex: orderWaypoints[i]?.waypoint_index ?? i,
            }))
            .sort((a, b) => a.osrmIndex - b.osrmIndex)
            .map(x => x.order)
          ...
          sortedByOsrm.forEach((order, i) => {
            ...
            markersRef.current.push(
              L.marker([order.lat!, order.lng!], { icon })
                .addTo(map)
                .bindPopup(`<b>#${i + 1} — ${order.order_code}</b><br>${order.customer_name}`)
            )
          })
        }
      }
      ...
      if (e.name === 'AbortError') {
        alert('OSRM tardó demasiado. Se guardará la ruta sin optimizar.')
      } else {
    ...
    // Verificar que hubo resultado antes de guardar
  ...
  const generarComprobante = (order: any) => {
    ...
    const fecha = new Date(order.created_at).toLocaleDateString('es-PE', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    })
    ...
    doc.text('Color', 100, 85)
    doc.text('Cant.', 135, 85)
    doc.text('Precio', 155, 85)
    doc.text('Subtotal', 175, 85)
    ...
    items.forEach((item: any) => {
      ...
      doc.text(String(item.color), 100, y)
    ...
    y += 8
    doc.setFont('helvetica', 'bold')
    doc.text('TOTAL: S/ ' + Number(order.total_amount).toFixed(2), 175, y, {
      align: 'right',
    })
    y += 12
    doc.setFont('helvetica', 'normal')
    doc.setFontSize(9)
    doc.text('Entrega: Motorizado', 20, y)

    if (order.destination) {
      y += 7
      doc.text('Direccion: ' + order.destination, 20, y)
    }

    if (order.reference) {
      y += 7
      doc.text('Referencia: ' + order.reference, 20, y)
    }
    ...
    <div>
      <div className="mb-4">
        ...
        <p className="text-gray-500 text-sm mt-0.5">
          Selecciona pedidos y genera una ruta optimizada
        </p>
        {storeOrigin ? (
          <p className="text-xs text-green-600 mt-1">🏪 Punto de salida configurado</p>
        ) : (
          <p className="text-xs text-orange-500 mt-1">
            ⚠️ Sin punto de salida — configúralo en{' '}
              ...
              {displayOrders.map((order) => {
                ...
                return (
                  <div
                    ...
                    className={`bg-white rounded-xl border p-3 lg:p-4 cursor-pointer transition-all touch-manipulation ${
                      selected.includes(order.id)
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-start gap-2 flex-1">
                        ...
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            ...
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full font-medium ${statusLabel[order.status]?.color}`}
                            >
                      ...
                      <div className="flex flex-col items-end gap-2">
                        <button
                          onClick={(e) => {
                            e.stopPropagation()
                            generarComprobante(order)
                          }}
                          className="px-2 py-1.5 bg-green-50 text-green-700 border border-green-200 rounded-lg text-xs font-medium touch-manipulation"
                        >
                          📄 PDF
                        </button>
            ...
            <>
              <div className="mt-3 flex gap-2">
                ...
                <button
                  ...
                  disabled={selected.length === 0 || optimizing}
                ...
                >
                  {optimizing ? '⏳ Optimizando...' : '🗺️ Generar ruta'}
                </button>
              </div>
                ...
                <div className="mt-3 bg-blue-50 border border-blue-200 rounded-xl px-4 py-3 flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-600">
                      {totalKm} km totales · {ordersWithLocation.length} paradas
                    </p>
                  </div>
                </div>
              )}
                ...
                <div className="mt-3 bg-green-50 border border-green-200 rounded-xl p-4">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <button
                      ...
                      className="py-2.5 rounded-xl text-xs font-medium bg-blue-600 text-white touch-manipulation"
                    >
                      📋 Copiar link
                    </button>
                    <button
                      onClick={() =>
                        window.open(
                          'https://wa.me/?text=' +
                            encodeURIComponent('Tu ruta de entregas: ' + routeLink),
                          '_blank'
                        )
                      }
                      className="py-2.5 rounded-xl text-xs font-medium bg-green-600 text-white touch-manipulation"
                    >
                      💬 WhatsApp
                    </button>
                  </div>
          ...
          {ordersWithoutLocation.length > 0 && selected.length > 0 && (
            <div className="mt-3 bg-orange-50 border border-orange-200 rounded-xl p-3">
              <p className="text-sm text-orange-700 font-medium">
                ⚠️ {ordersWithoutLocation.length} pedido(s) sin GPS no aparecen en el mapa
              </p>
              {ordersWithoutLocation.map((o) => (
                <p key={o.id} className="text-xs text-orange-600 mt-0.5">
                  • {o.order_code} — {o.destination}
                </p>
              ))}
            </div>
          )}
