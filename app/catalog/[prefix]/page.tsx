...
export default function CatalogPage({ params }: { params: Promise<{ prefix: string }> }) {
  ...
  const loadData = async (prefix: string) => {
    ...
    const { data: storeData } = await supabase
      .from('stores')
      .select(
        'id, name, phone, theme_color, logo_url, store_prefix, contact_whatsapp_msg, catalog_active'
      )
  ...
  const sendWhatsApp = () => {
    ...
    const lines = selected
      .map((s) => `• ${s.qty}x ${s.name} - S/ ${(s.price * s.qty).toFixed(2)}`)
      .join('\n')

    const msg =
      `¡Hola! Me interesa lo siguiente del catálogo:\n\n${lines}\n\n` +
          ...
          <div className="flex gap-2 mt-3">
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeTab === 'products' ? 'bg-white text-gray-800' : 'bg-white/20 text-white'
              }`}
            >
            ...
            <button
              ...
              className={`flex-1 py-2 rounded-lg text-sm font-medium transition-colors ${
                activeTab === 'combos' ? 'bg-white text-gray-800' : 'bg-white/20 text-white'
              }`}
            >
                      ...
                      {filteredProducts
                        ...
                        .map((product) => {
                          const isSelected = selected.some(
                            (s) => s.id === product.id && s.type === 'product'
                          )
                                ...
                                {isSelected ? (
                                  <div className="flex items-center justify-between mt-2">
                                    <div className="flex items-center gap-2">
                                      <button
                                        onClick={() => changeQty(product.id, 'product', -1)}
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {isSelected ? (
                            <div className="flex items-center gap-3 mt-3">
                              <div className="flex items-center gap-2">
                                <button
                                  onClick={() => changeQty(combo.id, 'combo', -1)}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-lg p-3 mb-2">
              <div className="flex items-center justify-between mb-2">
                <p className="text-xs font-semibold text-gray-600">
                  {totalSelected} item{totalSelected > 1 ? 's' : ''} seleccionado
                  {totalSelected > 1 ? 's' : ''}
                </p>
                <button onClick={() => setSelected([])} className="text-xs text-red-400 font-medium">
                  Limpiar todo
                </button>
              </div>
