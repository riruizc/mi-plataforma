...
export default function TrackPage() {
  ...
  const statusInfo = (status: string) => {
    if (status === 'pending')   return { label: 'Pendiente',  emoji: '🕐', color: '#d97706', bg: '#fef3c7', text: 'Tu pedido está siendo preparado.' }
    if (status === 'in_route')  return { label: 'En ruta',    emoji: '🛵', color: '#2563eb', bg: '#dbeafe', text: 'Tu pedido está en camino.' }
    if (status === 'delivered') return { label: 'Entregado',  emoji: '✅', color: '#059669', bg: '#d1fae5', text: 'Tu pedido fue entregado exitosamente.' }
    return { label: status, emoji: '📦', color: '#6b7280', bg: '#f3f4f6', text: '' }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      >
        <div className="max-w-lg mx-auto">
          {/* Logo + nombre tienda si hay pedido */}
          {order?.stores && (
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-white text-sm font-medium opacity-90">{order.stores.name}</span>
            </div>
          )}
          <h1 className="text-white font-bold text-xl">📦 Rastrear pedido</h1>
          <p className="text-white text-sm opacity-70 mt-0.5">Ingresa tu código para ver el estado</p>
        </div>
      </div>
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                ...
                <div className="p-4">
                  ...
                  >
                    ...
                    <p className="text-sm mt-1 opacity-80" style={{ color: info.color }}>{info.text}</p>
                  </div>
    ...
    return (
      <div key={i} className="flex flex-col items-center flex-1">
        ...
        <div className="flex items-center w-full">
          {/* Línea izquierda */}
          <div className={`flex-1 h-0.5 rounded-full ${i === 0 ? 'invisible' : active && i <= currentIndex ? 'bg-blue-600' : 'bg-gray-200'}`} />
          {/* Círculo */}
          ...
          {/* Línea derecha */}
          <div className={`flex-1 h-0.5 rounded-full ${i === 2 ? 'invisible' : i < currentIndex ? 'bg-blue-600' : 'bg-gray-200'}`} />
        </div>
        {/* Etiqueta */}
        <p className={`text-xs mt-1.5 font-medium text-center ${
          ...
          {label}
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-semibold text-gray-900">
                      {order.delivery_method === 'motorizado' ? '🛵 Motorizado' : '📦 Agencia'}
                    </span>
                  </div>
                  {order.agency_name && (
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-gray-500">Agencia</span>
                      <span className="text-sm font-semibold text-gray-900">{order.agency_name}</span>
                    </div>
                  )}
                  {order.destination && (
                    <div className="flex justify-between items-start gap-4">
                      <span className="text-sm text-gray-500 flex-shrink-0">Dirección</span>
                      <span className="text-sm font-medium text-gray-900 text-right">{order.destination}</span>
                    </div>
                  )}
                  {order.reference && (
                    <div className="flex justify-between items-start gap-4">
                      <span className="text-sm text-gray-500 flex-shrink-0">Referencia</span>
                      <span className="text-sm font-medium text-gray-900 text-right">{order.reference}</span>
                    </div>
                  )}
              ...
              {/* Botón buscar otro */}
              <button
                onClick={() => { setOrder(null); setCode(''); setError('') }}
