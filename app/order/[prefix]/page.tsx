        ...
        <div className="flex items-center justify-center gap-2 pt-3 pb-2 px-4">
          {store.logo_url && (
            <img
              src={store.logo_url}
              alt="Logo"
              className="w-8 h-8 rounded-full object-cover border-2 border-white border-opacity-60 flex-shrink-0"
            />
          )}
            ...
            <div key={s} className="flex items-center gap-1">
              <div
                className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                  step >= i + 1 ? 'bg-white' : 'bg-white bg-opacity-30'
                }`}
                style={{ color: step >= i + 1 ? store.theme_color || '#2563eb' : 'white' }}
              >
                {step > i + 1 ? '✓' : i + 1}
              </div>
              <span className={`text-white text-xs font-medium ${step === i + 1 ? 'opacity-100' : 'opacity-60'}`}>
                {s}
              </span>
                        ...
                        {product.variants.map((v) => {
                          ...
                          return (
                            <button
                              ...
                              className={`px-3 py-2 rounded-lg text-sm font-medium border transition-colors touch-manipulation ${
                                inCart
                ...
                <div className="max-w-lg mx-auto px-4 py-3 flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-gray-500">
                      {cart.reduce((s, c) => s + c.quantity, 0)} producto{cart.reduce((s, c) => s + c.quantity, 0) !== 1 ? 's' : ''}
                    </p>
        ...
        {step === 2 && (
          <div>
            ...
            <div className="flex gap-3 mt-4">
              <button
                onClick={() => setStep(1)}
                className="flex-1 py-3 rounded-xl font-semibold border border-gray-200 text-gray-700 touch-manipulation"
              >
                ← Atrás
              </button>
              <button
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className={`flex-1 py-3 rounded-xl text-sm font-semibold border-2 transition-colors touch-manipulation ${
              ...
              {delivery.method === 'motorizado' ? (
                    ...
                    <div className="absolute z-50 left-0 right-0 bg-white border border-gray-200 rounded-xl shadow-xl mt-1 max-h-52 overflow-y-auto">
                      {addressSuggestions.map((s: any) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            setDelivery((prev) => ({ ...prev, destination: s.display_name, lat: String(s.lat), lng: String(s.lon) }))
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <select
                      value={delivery.agency_name || ''}
                      onChange={(e) => setDelivery((prev) => ({ ...prev, agency_name: e.target.value, destination: '' }))}
                      className="w-full px-3 py-3 border border-gray-300 rounded-xl text-base focus:outline-none focus:ring-2 focus:ring-blue-500"
                    >
                      <option value="">Selecciona una agencia</option>
                      {agencies.map((a) => (
                        <option key={a.id} value={a.agency_name}>{a.agency_name}</option>
                      ))}
                    </select>
                  </div>
                    ...
                    <div>
                      ...
                      {agencies.find((a) => a.agency_name === delivery.agency_name)?.destinations?.length ? (
                        ...
                        >
                          ...
                          {agencies.find((a) => a.agency_name === delivery.agency_name)?.destinations.map((d: string) => (
              ...
              {delivery.method === 'motorizado' && (
                <div>
                  ...
                  {delivery.lat && delivery.lng && (
                    <p className="text-xs text-green-600 mt-2 font-medium">✅ Ubicación marcada</p>
                  )}
                </div>
              )}
            </div>
                  ...
                  <div className="flex items-center gap-3 flex-1 min-w-0">
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <button
                        ...
                        className="w-7 h-7 rounded-full bg-gray-100 text-gray-600 text-base font-bold flex items-center justify-center touch-manipulation active:bg-gray-200"
                      >
                        −
                      </button>
